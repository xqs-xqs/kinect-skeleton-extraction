				...
				for (int i = 0; i < iBodyCount; ++i)
				{
					...
					if ((pBody->get_IsTracked(&bTracked) == S_OK) && bTracked)
					{
						...
						if (pBody->GetJoints(JointType::JointType_Count, aJoints) == S_OK)
						{
							...
							DrawLine(mImg, aJoints[JointType_ElbowLeft], aJoints[JointType_ThumbLeft], pCoordinateMapper);
							...
							DrawLine(mImg, aJoints[JointType_ElbowRight], aJoints[JointType_ThumbRight], pCoordinateMapper);
